...
export function MobileGoalsHub() {
  ...
  const categories = useMemo(() => {
    if (!goals) return { fitness: [], biostack: [], habits: [], challenge: [] };
  ...
  const handleGoalClick = (goal: ChallengeGoal) => setSelectedGoal(goal);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-background">
      ...
      <GoalsSummaryHeader activeCount={stats.active} completedCount={stats.completed} totalPoints={totalPoints} />
      <FocusGoalsCarousel goals={focusGoals} onGoalClick={handleGoalClick} />
      <GoalCategoryTabs categories={categories} onGoalClick={handleGoalClick} />
      <FAB actions={[{ label: t('newGoal'), icon: Plus, onClick: () => setCreateDialogOpen(true), color: 'text-primary' }]} />
      <GoalCreateDialog open={createDialogOpen} onOpenChange={setCreateDialogOpen} onGoalCreated={() => refetch()} />
      {selectedGoal && (
        <QuickMeasurementDialog
          goal={selectedGoal}
          isOpen={!!selectedGoal}
          onOpenChange={(open) => !open && setSelectedGoal(null)}
          onMeasurementAdded={() => { setSelectedGoal(null); refetch(); }}
        />
      )}
    </motion.div>
  );
