...
export function ParticipantPodium({ topThree }: ParticipantPodiumProps) {
  ...
  const getPodiumGradient = (position: number) => {
    switch (position) {
      case 0: return "from-gold/20 to-gold/5"; // Gold
      case 1: return "from-silver/20 to-silver/5"; // Silver
      case 2: return "from-bronze/20 to-bronze/5"; // Bronze
        ...
        {podiumOrder.map((participant, visualIndex) => {
          ...
          return (
            <div
              ...
              className={cn(
                "flex flex-col items-center gap-3 hover-lift",
                visualIndex === 1 ? "scale-110" : "" // Center (1st place) is larger
              )}
            >
              {/* Medal Icon */}
              <div className={cn("animate-bounce-in", getMedalGlow(originalIndex))}>
                {getMedalIcon(originalIndex)}
              </div>
              ...
              <div className="relative">
                <Avatar className={cn(
                  "h-20 w-20 border-4",
                  originalIndex === 0 && "border-gold",
                  originalIndex === 1 && "border-silver",
                  originalIndex === 2 && "border-bronze",
                  getMedalGlow(originalIndex)
                )}>
                  <AvatarImage src={participant.profiles?.avatar_url} />
                  <AvatarFallback className={cn(
                    "text-xl font-bold",
                    originalIndex === 0 && "bg-gold/20 text-gold",
                    originalIndex === 1 && "bg-silver/20 text-silver",
                    originalIndex === 2 && "bg-bronze/20 text-bronze"
                  )}>
                    {participant.profiles?.username?.[0]?.toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                {participant.profiles?.trainer_role && (
                  <Badge className="absolute -top-2 -right-2 bg-gradient-primary text-white text-xs px-1.5 py-0.5">
                    Trainer
                  </Badge>
                )}
              </div>
              ...
              <div
                className={cn(
                  "w-32 rounded-t-xl bg-gradient-to-b border-t-2 flex flex-col items-center justify-center gap-2 transition-all",
                  getPodiumHeight(originalIndex),
                  getPodiumGradient(originalIndex),
                  originalIndex === 0 && "border-gold",
                  originalIndex === 1 && "border-silver",
                  originalIndex === 2 && "border-bronze"
                )}
              >
                ...
                <div className="flex items-center gap-1">
                  <Star className={cn(
                    "h-5 w-5",
                    originalIndex === 0 && "text-gold",
                    originalIndex === 1 && "text-silver",
                    originalIndex === 2 && "text-bronze"
                  )} />
                  <span className="text-2xl font-bold">{participant.points}</span>
                </div>

                {/* Streak */}
                {participant.streak_days > 0 && (
                  <div className="flex items-center gap-1 text-sm">
                    <Flame className="h-4 w-4 text-orange-500" />
                    <span>{participant.streak_days} days</span>
                  </div>
                )}
