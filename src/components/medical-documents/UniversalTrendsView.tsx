...
import { Droplet, Activity, Stethoscope, TrendingUp } from "lucide-react";
...
export const UniversalTrendsView = () => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-3xl font-bold mb-2">Тренды и аналитика</h1>
        <p className="text-muted-foreground">
          Визуализация изменений ваших показателей здоровья
        </p>
      </div>
      ...
      <Tabs value={activeCategory} onValueChange={setActiveCategory} className="space-y-4">
        <TabsList className="glass-card grid w-full grid-cols-3 h-auto p-1">
          <TabsTrigger value="blood" className="gap-2 data-[state=active]:bg-red-500/20">
            ...
            <span className="hidden sm:inline">Анализы крови</span>
          </TabsTrigger>
          <TabsTrigger value="body" className="gap-2 data-[state=active]:bg-blue-500/20">
            <Activity className="h-4 w-4" />
            <span className="hidden sm:inline">Состав тела</span>
          </TabsTrigger>
