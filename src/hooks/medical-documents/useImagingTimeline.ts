...
export function useImagingTimeline() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const docIdsWithFindings = findingsData 
        ? [...new Set(findingsData.map((f: { document_id: string }) => f.document_id))]
        : [];

      // Build the OR condition including documents with findings
      const keywordConditions = [
        'category.eq.imaging_report',
        'file_name.ilike.%узи%',
        'file_name.ilike.%мрт%',
        'file_name.ilike.%кт%',
        'file_name.ilike.%рентген%',
        'file_name.ilike.%ultrasound%',
        'file_name.ilike.%mri%',
        'file_name.ilike.%ct scan%',
        'file_name.ilike.%x-ray%',
        'file_name.ilike.%органов%',
        'file_name.ilike.%gastro%',
        'file_name.ilike.%эндоскопия%',
        'file_name.ilike.%endoscopy%',
        'file_name.ilike.%колоноскопия%',
        'file_name.ilike.%эхокг%'
      ];
      
      // Add document IDs with findings to the OR condition
      if (docIdsWithFindings.length > 0) {
        keywordConditions.push(`id.in.(${docIdsWithFindings.join(',')})`);
      }
