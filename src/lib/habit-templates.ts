...
// Legacy: Keep DEFAULT_HABIT_TEMPLATES for backward compatibility
// These use hardcoded Russian strings for existing code that doesn't use i18n
export const DEFAULT_HABIT_TEMPLATES: HabitTemplate[] = [
  ...
  },
  {
    ...
    habit_type: 'duration_counter',
    category: 'health',
    description: 'Ясность ума и здоровье',
    custom_settings: {
      cost_per_day: 500,
      show_health_benefits: true
    },
    ai_motivation: {
      milestones: {
        1440: '🧠 Через 24 часа ясность ума возвращается',
        4320: '💤 Через 3 дня сон нормализуется',
        10080: '💪 Через неделю память улучшается',
        20160: '🎯 Через 2 недели концентрация на пике',
        43200: '🌟 Через месяц легкие очищаются',
        129600: '🏆 Через 3 месяца когнитивные функции восстановлены',
        259200: '🎉 Через полгода полное восстановление'
      }
    }
  }
];
...
export const getHabitTemplate = (templateId: string): HabitTemplate | undefined => {
  // Try localized first, fall back to legacy
  const localized = getLocalizedTemplates().find(t => t.id === templateId);
  return localized || DEFAULT_HABIT_TEMPLATES.find(t => t.id === templateId);
};

export const getDefaultHabits = (): HabitTemplate[] => {
  // Return localized templates if i18n is ready, otherwise legacy
  try {
