/**
 * Challenge Baseline Utilities
 * Capture and manage baseline metrics for challenge participants
 */

import { supabase } from '@/integrations/supabase/client';

/**
 * Capture baseline body composition data for a challenge participant
 * Tries InBody first, then falls back to Withings
 */
...
/**
 * Check if baseline exists for a participant
 */
export async function hasBaseline(
  ...
  const { data } = await supabase
    .from('challenge_participants')
    .select('baseline_body_fat, baseline_weight')
    .eq('user_id', userId)
    .eq('challenge_id', challengeId)
    .single();

  return !!(data?.baseline_body_fat || data?.baseline_weight);
}

/**
 * Get baseline data for a participant
 */
export async function getBaseline(userId: string, challengeId: string) {
