-- Update the create_activity_feed_entry function to properly handle Sleep Duration
  ...
  ELSIF TG_TABLE_NAME = 'metric_values' THEN
    ...
    IF metric_info.metric_category IN ('workout', 'recovery', 'sleep', 'cardio') THEN
      ...
      ELSIF metric_info.metric_category = 'recovery' THEN
        ...
        FROM public.activity_feed
        WHERE user_id = NEW.user_id
          AND action_text LIKE '%recovered%'
          AND DATE(created_at) = NEW.measurement_date;
      ...
      ELSIF metric_info.metric_category = 'sleep' THEN
        ...
        IF metric_info.metric_name = 'Sleep Duration' THEN
          ...
          BEGIN
            ...
            action_text := action_text || ' slept ' || formatted_time;
          END;
        ELSE
          -- For other sleep metrics, aggregate as before
          SELECT id, 
                 CAST(SUBSTRING(action_text FROM 'slept ([0-9.]+)h') AS NUMERIC) INTO existing_entry_id, existing_sleep_hours
          FROM public.activity_feed
          WHERE user_id = NEW.user_id
            AND action_text LIKE '%slept%'
            AND DATE(created_at) = NEW.measurement_date
          ...
          action_text := action_text || ' slept ' || total_sleep_hours || 'h';
        END IF;
