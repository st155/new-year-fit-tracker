-- Исправление materialized view проблемы
-- Заменяем materialized view на обычный view для автоматического обновления данных

-- Удалить materialized view
DROP MATERIALIZED VIEW IF EXISTS public.trainer_client_summary;

-- Создать обычный view вместо materialized
...
-- Удалить функцию обновления materialized view (больше не нужна)
DROP FUNCTION IF EXISTS public.refresh_trainer_client_summary();

-- Предоставить доступ к view
