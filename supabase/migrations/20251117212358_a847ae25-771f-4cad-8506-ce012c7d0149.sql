-- Update get_client_detailed_data to prioritize longest sleep duration
CREATE OR REPLACE FUNCTION public.get_client_detailed_data(p_client_id UUID, p_days INTEGER DEFAULT 30)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  RETURN jsonb_build_object(
    ...
    'unified_metrics', (
      ...
      ) ORDER BY um.measurement_date DESC, um.priority ASC, um.value DESC), '[]'::jsonb)
    ...
    'workouts', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        'notes', w.notes
      ) ORDER BY w.start_time DESC), '[]'::jsonb)
      FROM workouts w
