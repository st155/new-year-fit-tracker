-- Create RPC to get client goals with progress calculated from measurements
...
-- Normalize existing units in goals table
UPDATE goals 
SET target_unit = 'раз' 
WHERE target_unit IN ('reps', 'reps=');

UPDATE goals 
SET target_unit = 'кг' 
WHERE target_unit LIKE 'кг×%';

-- Normalize existing units in measurements table
UPDATE measurements 
SET unit = 'раз' 
WHERE unit IN ('reps', 'reps=');
