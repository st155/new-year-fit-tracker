-- 1) Update function to skip workout entries for metric_values to avoid duplicates
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'metric_values' THEN
    ...
    -- IMPORTANT: Skip workout category entirely to avoid duplicates with workouts
...
-- 2) Ensure a single trigger exists only for workouts to avoid multiple entries
DO $$
BEGIN
  IF EXISTS (
    ...
    WHERE NOT tgisinternal
      ...
      AND tgname <> 'trg_activity_feed_workouts'
  ) THEN
    -- Drop any non-standard triggers that may call the same function
    PERFORM 1;
  END IF;
END $$;

-- Drop and recreate the workouts trigger to be safe
...
-- 3) Cleanup: collapse duplicate workout-like entries created via metric_values
-- Keep only the latest per external_id (or per user+measurement_date if no external_id)
WITH to_delete AS (
  SELECT id FROM (
    SELECT 
      af.id,
      ROW_NUMBER() OVER (
        PARTITION BY COALESCE(af.metadata->>'external_id', af.user_id::text || '|' || (af.metadata->>'measurement_date'))
        ORDER BY af.created_at DESC
      ) AS rn
    FROM public.activity_feed af
    WHERE af.source_table = 'metric_values'
      AND (
        af.action_text ILIKE '%completed a workout%'
        OR af.action_text ILIKE '%трениров%'
      )
  ) t
  WHERE rn > 1
)
