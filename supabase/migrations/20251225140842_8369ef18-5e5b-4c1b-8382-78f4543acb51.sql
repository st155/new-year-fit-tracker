...
-- Fix connected_sources to use terra_tokens instead of unified_metrics
-- This ensures we show ACTIVE integrations, not just sources with recent data
...
BEGIN
  ...
  -- FIXED: Use terra_tokens for active integrations instead of unified_metrics
  connected_sources_data AS (
    SELECT
      cd.cid,
      ARRAY_AGG(DISTINCT UPPER(tt.provider)) FILTER (
        WHERE tt.provider IS NOT NULL
      ) as sources
    FROM client_data cd
    LEFT JOIN terra_tokens tt ON tt.user_id = cd.cid AND tt.is_active = true
    GROUP BY cd.cid
  ),
  health_metrics AS (
    SELECT
