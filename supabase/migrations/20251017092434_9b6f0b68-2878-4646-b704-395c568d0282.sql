-- Исправление триггера create_activity_feed_entry для workouts
-- Проблема: триггер пытается использовать measurement_date, которого нет в таблице workouts
-- Решение: использовать DATE(start_time) вместо measurement_date
...
BEGIN
  ...
  -- Исправлено: для workouts используем DATE(start_time)
  ...
  IF TG_TABLE_NAME = 'workouts' THEN
    ...
    -- Используем DATE(start_time) вместо measurement_date
