-- Fix metric names in challenge_leaderboard_v2 to match actual data in unified_metrics
-- Previous migration used lowercase names, but metrics are stored with Title Case
...
WITH user_activity AS (
  SELECT 
    ...
    -- Activity volume metrics (FIXED: Using correct Title Case metric names)
    SUM(CASE WHEN um.metric_name = 'Steps' THEN um.value ELSE 0 END) as total_steps,
    SUM(CASE WHEN um.metric_name = 'Active Calories' THEN um.value ELSE 0 END) as total_active_calories,
    -- Quality metrics (FIXED: Using correct Title Case metric names)
