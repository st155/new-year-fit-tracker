-- Add connected sources to get_trainer_clients_enhanced
-- This will show which integrations (Whoop, Oura, Garmin, Withings) each client has connected
...
RETURNS TABLE (
  ...
  top_3_goals JSONB,
  connected_sources TEXT[] -- NEW: Array of connected data sources
) 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  SELECT 
    ...
    COALESCE(csd.sources, ARRAY[]::TEXT[]) as connected_sources -- NEW
