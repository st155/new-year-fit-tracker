-- Fix create_activity_feed_entry function to prevent JSON to numeric errors and duplicates
...
-- Recreate triggers
DROP TRIGGER IF EXISTS create_activity_on_workout ON workouts;
CREATE TRIGGER create_activity_on_workout
  AFTER INSERT ON workouts
  FOR EACH ROW
  EXECUTE FUNCTION create_activity_feed_entry();

DROP TRIGGER IF EXISTS create_activity_on_measurement ON measurements;
CREATE TRIGGER create_activity_on_measurement
  AFTER INSERT ON measurements
  FOR EACH ROW
  EXECUTE FUNCTION create_activity_feed_entry();

DROP TRIGGER IF EXISTS create_activity_on_body_composition ON body_composition;
CREATE TRIGGER create_activity_on_body_composition
  AFTER INSERT ON body_composition
  FOR EACH ROW
  EXECUTE FUNCTION create_activity_feed_entry();

DROP TRIGGER IF EXISTS create_activity_on_goal ON goals;
CREATE TRIGGER create_activity_on_goal
  AFTER INSERT ON goals
  FOR EACH ROW
  EXECUTE FUNCTION create_activity_feed_entry();

DROP TRIGGER IF EXISTS create_activity_on_metric_value ON metric_values;
CREATE TRIGGER create_activity_on_metric_value
