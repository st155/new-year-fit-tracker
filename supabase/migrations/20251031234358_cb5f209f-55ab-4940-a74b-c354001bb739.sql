-- Fix type mismatch in get_client_goals_with_progress RPC function
-- Issue: Function returns DATE but was expecting TIMESTAMPTZ for last_measurement_date
...
BEGIN
  ...
  SELECT 
    ...
    (SELECT MAX(m.measurement_date) FROM measurements m WHERE m.goal_id = g.id)::DATE as last_measurement_date,
